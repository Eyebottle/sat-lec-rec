...
## 🚀 **추천!** 가장 간단한 방법: 원클릭 전체 자동 테스트

### 특징
- ✅ **가장 간단**: PMI 링크만 있으면 OK (OAuth 앱 불필요!)
- ✅ **완전 자동**: 버튼 한 번으로 7단계 자동 실행 (25초)
- ✅ **반복 사용**: 한 번 저장하면 계속 재사용 가능
- ✅ **안전**: 본인 PMI만 사용하므로 실제 강의 방해 없음

### 사용 방법

#### 1️⃣ PMI 링크 찾기 (5분, 1회만)

**방법 A: Zoom 웹사이트**
```
1. https://zoom.us/profile 접속
2. 로그인
3. "개인 회의 ID (PMI)" 섹션에서 링크 확인
...
#### 2️⃣ 앱에 링크 저장 (30초, 1회만)

```
1. sat-lec-rec 앱 실행
2. ⚙️ 설정 버튼 클릭
3. "Zoom 설정" 카드에서 "테스트용 Zoom 링크" 입력
4. PMI 링크 붙여넣기
5. "저장" 버튼 클릭
```

#### 3️⃣ 원클릭 테스트 실행 (25초, 언제든지!)

```
1. 🐛 버튼 클릭 (우측 상단 디버그 버튼)
2. "🚀 원클릭 전체 자동 테스트" 녹색 카드 찾기
3. "전체 자동 테스트 시작" 버튼 클릭
4. 25초간 7단계 자동 실행:
   - 1/7 🔵 Zoom 실행
   - 2/7 🟢 자동 참가 (이름 입력 + 버튼 클릭)
   - 3/7 🔊 오디오 참가
   - 4/7 📹 비디오 끄기
   - 5/7 🔇 음소거 설정
   - 6/7 ⏱️ 10초 안정성 확인
...
| 항목 | 원클릭 테스트 | Zoom API | 수동 테스트 |
|------|--------------|----------|------------|
| 설정 시간 | **5분** | 30분+ | 0분 |
| OAuth 앱 필요 | ❌ | ✅ | ❌ |
| 자동화 정도 | **완전 자동** | 완전 자동 | 수동 |
| 반복 사용 | **버튼 1회** | 버튼 1회 | 매번 수동 |
| 실제 강의 안전 | ✅ | ✅ | ⚠️ |

**결론**: 대부분의 사용자에게 "원클릭 테스트"를 추천합니다!

---

## 🤖 고급 사용자용: Zoom API 자동 회의 생성
